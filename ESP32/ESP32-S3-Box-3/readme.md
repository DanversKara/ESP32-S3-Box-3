So i am in the middle of testing a new Firmware designed voip_stack HACS but isnt even done or released yet but if you have an ESP32-S3-Box-3 you can try right now Firmware designed FreePBX or IncrediblePBX or Asterisk, i also have a Start-From-Scartch-Code that lets you build from scratch, so you dont need to have Ai or self remove code and break code.

Firmware designed FreePBX or IncrediblePBX or Asterisk - allows users to connect directly to their fav sip trunk outside HA while Firmware designed voip_stack HACS is designed to work only with HA and all its HACS/APPS.
