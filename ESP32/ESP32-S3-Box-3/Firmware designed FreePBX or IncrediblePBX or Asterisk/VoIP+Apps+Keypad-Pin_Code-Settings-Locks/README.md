...
substitutions:
  ...
  light_widget_entity: light.living_room_living_room_light
  menu_fan_entity: fan.living_room_fan
  menu_tv_entity: switch.living_room_tv_ir
  menu_soundbar_entity: switch.living_room_soundbar
```
...
**Door lock** — A PIN-pad page gates access to `lock.living_room_front_door_lock` when "Require PIN for Lock" is enabled in Settings; otherwise the lock toggles directly.

**Lights** — Brightness and color-temperature sliders write to your configured `light_widget_entity` and stay in sync if the light is changed from elsewhere (physical switch, HA automation, etc.).

**IR menu** — Toggle buttons for TV, soundbar, and fan, mirroring the real Home Assistant switch state (button recolors if changed externally).

**Settings page** — Master volume (+/‑ buttons), backlight brightness slider, speaker mute switch, PIN-lock toggle for the door, and the ringtone test button. Entry to this page is PIN-protected.
